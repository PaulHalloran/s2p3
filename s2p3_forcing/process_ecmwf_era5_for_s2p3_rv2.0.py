# note - ECMWF fluxes are accumulations since teh start of the forcast period, see https://www.ecmwf.int/sites/default/files/elibrary/2015/18490-radiation-quantities-ecmwf-model-and-mars.pdf
##################################
# importing modules we will be using later
##################################

print 'The accumulations (mean values) of ERA5 are treated differently compared with those in ERA-Interim and operational data (where they are from the beginning of the forecast to the forecast step). Here ther are hourly means for most things.'
print 'see https://confluence.ecmwf.int/display/CKB/ERA5+data+documentation#ERA5datadocumentation-Meanratesandaccumulations'
...
domain_file = 's12_m2_s2_n2_h_map_1801803030_0.1.dat'
...
cube = iris.load_cube(directory_containing_files_to_process + input_variables[0]+'.nc')
try:
...
exists = os.path.isfile(directory_containing_files_to_process + 'wind_speed.nc')
if not exists:
...
for year in range(start_year,end_year+1):
    ...
    for k in range(len(input_variables)):
        ...
        cube_year.data = np.flip(cube_year.data,axis=1)
    ...
    mask_files = glob.glob(directory_containing_files_to_process + 'lsmask.nc')
    if len(mask_files) == 0:
